    ...
    switch (action.type) {
        ...
        case OPEN_CELL: {
            ...
            if (state.data.row * state.data.cell - state.data.mine === state.openedCount + openedCount) {
                // 승리
                halted = true;
                result = `승리하셨습니다. 클리어 시간은 ${state.timer}초입니다.`;
            }
            return {
                ...state,
                tableData,
...
const MineSearch = () => {
    ...
    const value = useMemo(() => ({ tableData, halted, dispatch }), [tableData, halted]); // useMemo를 통해 캐싱을 해줘야 성능 저하가 덜 일어남, dispatch는 항상 같게 유지되므로 2번째 인자에 추가하지 않아도 됨
